...
- 输出图：`main_visualization.png`（累计收益、风险指数、仓位 三图）；`analysis_charts.png`（风险分布饼图、多尺度准确率、策略绩效表，无分年度收益图）；`forward_10d_quantile_timeseries.png`（累计收益）；`forward_10d_quantile_price_timeseries.png`（WTI 远期收盘价，由预测累计收益 × 基准价映射，文件名随 REGRESSION_HORIZON 变化）
- 并联模型：分位数回归预测未来 REGRESSION_HORIZON 个交易日累计收益（点预测=中位数，区间=10%/90% 分位）
"""
...
REGRESSION_TARGET_COL = "Forward_CumRet_10d"
PRICE_COL = "WTI_Close"
FC_QUANTILES = (0.1, 0.5, 0.9)
QUANTILE_REG_ALPHA = 0.05
...
def simulate_weekly_strategy(y_pred, y_true, consensus_list, config):
    ...
    annual_ret = -1.0 if (1 + total_ret) <= 0 else float((1 + total_ret) ** (252 / len(y_true)) - 1)
...
def load_and_prepare_data(file_path):
    ...
    # 并联回归目标：自交易日 t 起的 REGRESSION_HORIZON 日内累计收益（复利）
    r = df[primary_target].values.astype(float)
    h = REGRESSION_HORIZON
    fc = np.full(len(r), np.nan)
    for t in range(len(r) - h + 1):
        fc[t] = float(np.prod(1.0 + r[t : t + h]) - 1.0)
    df[REGRESSION_TARGET_COL] = fc

    print(f"总特征数（含工程特征，不含目标）: {len([c for c in df.columns if c not in targets and c != REGRESSION_TARGET_COL])}")
...
def train_quantile_forward_predict(X_train, y_train, X_test, quantiles=FC_QUANTILES, alpha=QUANTILE_REG_ALPHA):
    """
    分位数回归（并联）：对连续累计收益给分位点预测；中位数作点预测，低/高分位作区间。
    返回 (q低, q50, q高) 三个与 X_test 等长的数组，且逐点满足 q低<=q50<=q高。
    """
    ...
    X_tr = np.nan_to_num(X_train[mask], nan=0, posinf=0, neginf=0)
    y_tr = y_train[mask]
    X_te = np.nan_to_num(X_test, nan=0, posinf=0, neginf=0)
    ...
    for q in quantiles:
        try:
            try:
                mdl = QuantileRegressor(quantile=q, alpha=alpha, solver="highs")
            except TypeError:
                mdl = QuantileRegressor(quantile=q, alpha=alpha)
            mdl.fit(X_tr_s, y_tr)
            preds.append(mdl.predict(X_te_s))
        except Exception:
            preds.append(np.full(n_te, float(np.median(y_tr))))

    a, b, c = preds[0], preds[1], preds[2]
    p_lo = np.minimum(np.minimum(a, b), c)
    p_hi = np.maximum(np.maximum(a, b), c)
    p_mid = np.clip(b, p_lo, p_hi)
...
def rolling_prediction(df, primary_target, targets, base_cols, top_n=30, train_window=756, period_days=30):
    ...
    all_feature_cols = [
        c for c in df.columns if c not in set(targets) and c != REGRESSION_TARGET_COL
    ]
    n_samples = len(df)
    ...
    results = {
        ...
        'actual_fc10': [],
        'pred_fc10_q10': [], 'pred_fc10_q50': [], 'pred_fc10_q90': [],
        'fwd_base_close': [],
        'actual_fwd_close': [],
        'selected_factors': [],
        'period_info': []
    ...
    while current_idx < n_samples:
        ...
        train_min_len = min(len(X_train), len(y_train_1d))
        X_train = X_train[-train_min_len:]
        y_train_1d = y_train_1d[-train_min_len:]
        y_train_5d = y_train_5d[-train_min_len:]
        y_train_20d = y_train_20d[-train_min_len:]
        ...
        test_min_len = min(len(X_test), len(y_test_1d))
        X_test = X_test[:test_min_len]
        y_test_1d = y_test_1d[:test_min_len]
        y_test_5d = y_test_5d[:test_min_len]
        y_test_20d = y_test_20d[:test_min_len]
        test_dates = test_dates[:test_min_len]
        ...
        y_train_fc = df[REGRESSION_TARGET_COL].iloc[train_start : train_end + 1].values
        y_train_fc = y_train_fc[-train_min_len:]
        y_test_fc = df[REGRESSION_TARGET_COL].iloc[test_start : test_end + 1].values[:test_min_len]

        fc_p10, fc_p50, fc_p90 = train_quantile_forward_predict(X_train, y_train_fc, X_test)
        ...
        for i in range(len(y_pred_1d)):
            ...
            results['actual_fc10'].append(float(y_test_fc[i]) if np.isfinite(y_test_fc[i]) else np.nan)
            results['pred_fc10_q10'].append(float(fc_p10[i]))
            results['pred_fc10_q50'].append(float(fc_p50[i]))
            results['pred_fc10_q90'].append(float(fc_p90[i]))
            t_i = test_start + i
            end_close_idx = t_i + REGRESSION_HORIZON - 1
            if t_i >= 1:
                base_c = float(df[PRICE_COL].iloc[t_i - 1])
            else:
                base_c = np.nan
            if end_close_idx < len(df):
                act_c = float(df[PRICE_COL].iloc[end_close_idx])
            else:
                act_c = np.nan
            results['fwd_base_close'].append(base_c)
            results['actual_fwd_close'].append(act_c if np.isfinite(act_c) else np.nan)
...
def plot_forward_quantile_timeseries(results, output_dir):
    """并联分位数回归：未来 REGRESSION_HORIZON 日累计收益 — 实值 vs 预测区间（时间序列）。"""
    dates = pd.to_datetime(results["dates"])
    act = np.asarray(results["actual_fc10"], dtype=float)
    q10 = np.asarray(results["pred_fc10_q10"], dtype=float)
    q50 = np.asarray(results["pred_fc10_q50"], dtype=float)
    q90 = np.asarray(results["pred_fc10_q90"], dtype=float)
    if len(dates) == 0:
        return
    ...
    ax.fill_between(dates, lo, hi, color="#90cdf4", alpha=0.4, label="预测区间 Q10–Q90")
    ax.plot(dates, q50, color="#2b6cb0", linewidth=1.2, label="预测中位数 Q50")
    m = np.isfinite(act)
    if np.any(m):
        ax.plot(
            np.array(dates)[m],
            ...
            linewidth=1.0,
            alpha=0.92,
            label=f"实际 {REGRESSION_HORIZON}日累计收益",
        )
    ax.axhline(0, color="k", linestyle="--", linewidth=0.5)
    ax.set_ylabel("累计收益（小数）", fontsize=11)
    ax.set_xlabel("日期", fontsize=11)
    ax.set_title(
        f"并联分位数回归｜未来{REGRESSION_HORIZON}个交易日累计收益：实值 vs 预测区间",
        fontsize=13,
        fontweight="bold",
    )
    ...
    plt.tight_layout()
    out_path = f"{output_dir}/forward_{REGRESSION_HORIZON}d_quantile_timeseries.png"
    plt.savefig(out_path, dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor())
    print(f"  并联预测时序图: {out_path}")
    plt.close()


def plot_forward_quantile_price_timeseries(results, output_dir):
    """
    将预测的 h 日复利累计收益映射为远期收盘价：P_fwd = P_{t-1} × (1 + fc_pred)，
    与目标定义 fc[t] = Close[t+h-1]/Close[t-1] - 1 一致（P_{t-1} 为预测日前一交易日收盘）。
    """
    dates = pd.to_datetime(results["dates"])
    base = np.asarray(results["fwd_base_close"], dtype=float)
    act_px = np.asarray(results["actual_fwd_close"], dtype=float)
    q10 = np.asarray(results["pred_fc10_q10"], dtype=float)
    q50 = np.asarray(results["pred_fc10_q50"], dtype=float)
    q90 = np.asarray(results["pred_fc10_q90"], dtype=float)
    if len(dates) == 0 or len(base) == 0:
        return
    lo_ret = np.minimum(q10, q90)
    hi_ret = np.maximum(q10, q90)
    pred_lo = base * (1.0 + lo_ret)
    pred_hi = base * (1.0 + hi_ret)
    pred_mid = base * (1.0 + q50)
    band_lo = np.minimum(pred_lo, pred_hi)
    band_hi = np.maximum(pred_lo, pred_hi)

    fig, ax = plt.subplots(figsize=(14, 5.5), facecolor="white")
    ax.fill_between(dates, band_lo, band_hi, color="#9ae6b4", alpha=0.4, label="预测区间（价格）Q10–Q90")
    ax.plot(dates, pred_mid, color="#276749", linewidth=1.2, label="预测中位价格 Q50")
    m = np.isfinite(act_px)
    if np.any(m):
        ax.plot(
            np.array(dates)[m],
            act_px[m],
            ...
            linewidth=1.0,
            alpha=0.92,
            label=f"实际收盘（t+{REGRESSION_HORIZON}）",
        )
    ax.set_ylabel(f"{PRICE_COL}（美元/桶）", fontsize=11)
    ax.set_xlabel("预测日 t（对应特征对齐日）", fontsize=11)
    ax.set_title(
        f"并联分位数回归｜{PRICE_COL} 远期价格：基准为 t−1 收盘 × (1+预测累计收益)",
        fontsize=13,
        fontweight="bold",
    )
    ax.legend(loc="upper left", framealpha=0.92)
    ...
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m"))
    plt.tight_layout()
    out_path = f"{output_dir}/forward_{REGRESSION_HORIZON}d_quantile_price_timeseries.png"
    plt.savefig(out_path, dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor())
    print(f"  并联价格预测时序图: {out_path}")
    plt.close()
...
def create_visualizations(results, output_dir):
    ...
    plot_forward_quantile_timeseries(results, output_dir)
    plot_forward_quantile_price_timeseries(results, output_dir)
...
def evaluate_and_save(results, output_dir='final_solution'):
    ...
    annual_ret = -1.0 if (1 + total_ret) <= 0 else (1 + total_ret) ** (252 / len(y_true)) - 1
    ...
    act_fc = np.asarray(results["actual_fc10"], dtype=float)
    q10_fc = np.asarray(results["pred_fc10_q10"], dtype=float)
    q50_fc = np.asarray(results["pred_fc10_q50"], dtype=float)
    q90_fc = np.asarray(results["pred_fc10_q90"], dtype=float)
    m_fc = np.isfinite(act_fc)
    if np.any(m_fc):
        fc_interval_cover = float(np.mean((act_fc[m_fc] >= q10_fc[m_fc]) & (act_fc[m_fc] <= q90_fc[m_fc])))
        fc_mae_median = float(np.mean(np.abs(act_fc[m_fc] - q50_fc[m_fc])))
        fc_rmse = float(np.sqrt(np.mean((act_fc[m_fc] - q50_fc[m_fc]) ** 2)))
    else:
        fc_interval_cover = fc_mae_median = fc_rmse = float("nan")

    pd.DataFrame(
        [
            {
                "horizon_trading_days": REGRESSION_HORIZON,
                "q_low": FC_QUANTILES[0],
                "q_mid": FC_QUANTILES[1],
                "q_high": FC_QUANTILES[2],
                "empirical_interval_coverage": fc_interval_cover,
                "mae_median_pred": fc_mae_median,
                "rmse_median_pred": fc_rmse,
                "n_valid": int(np.sum(m_fc)),
            }
        ]
    ).to_csv(
        f"{output_dir}/forward_{REGRESSION_HORIZON}d_quantile_summary.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    print(f"\n【并联】未来{REGRESSION_HORIZON}日累计收益 · 分位数回归")
    print(f"  名义80%区间覆盖(实值落在[Q10,Q90]比例): {fc_interval_cover:.2%}")
    print(f"  中位预测 MAE / RMSE: {fc_mae_median:.4f} / {fc_rmse:.4f}（相对累计收益小数）")
    print(f"\n【多时间尺度准确率】")
    ...
    base_c = np.asarray(results["fwd_base_close"], dtype=float)
    q10a = np.asarray(results["pred_fc10_q10"], dtype=float)
    q50a = np.asarray(results["pred_fc10_q50"], dtype=float)
    q90a = np.asarray(results["pred_fc10_q90"], dtype=float)
    pred_px_q10 = base_c * (1.0 + q10a)
    pred_px_q50 = base_c * (1.0 + q50a)
    pred_px_q90 = base_c * (1.0 + q90a)
    ...
    results_df = pd.DataFrame({
        ...
        f'Actual_Fwd{REGRESSION_HORIZON}d_Cum': results['actual_fc10'],
        f'Pred_Fwd{REGRESSION_HORIZON}d_Q10': results['pred_fc10_q10'],
        f'Pred_Fwd{REGRESSION_HORIZON}d_Q50': results['pred_fc10_q50'],
        f'Pred_Fwd{REGRESSION_HORIZON}d_Q90': results['pred_fc10_q90'],
        f'Fwd_Base_{PRICE_COL}': results['fwd_base_close'],
        f'Actual_Fwd_{REGRESSION_HORIZON}d_Close': results['actual_fwd_close'],
        f'Pred_Fwd_{REGRESSION_HORIZON}d_Close_Q10': pred_px_q10,
        f'Pred_Fwd_{REGRESSION_HORIZON}d_Close_Q50': pred_px_q50,
        f'Pred_Fwd_{REGRESSION_HORIZON}d_Close_Q90': pred_px_q90,
    ...
    with open(f'{output_dir}/report.txt', 'w') as f:
        ...
        if STRATEGY_SEARCH_SUMMARY:
            f.write("\n" + STRATEGY_SEARCH_SUMMARY + "\n\n")
        f.write(f"【并联分位数回归｜未来{REGRESSION_HORIZON}日累计收益】\n")
        f.write("  目标: 自预测日起连续10个交易日简单收益的复利累计\n")
        f.write("  模型: sklearn QuantileRegressor（10%/50%/90%），与逻辑回归共用ICIR因子与对齐方式\n")
        f.write(f"  区间覆盖(实值∈[Q10,Q90]): {fc_interval_cover:.2%}\n")
        f.write(f"  中位预测 MAE: {fc_mae_median:.6f}  RMSE: {fc_rmse:.6f}\n\n")

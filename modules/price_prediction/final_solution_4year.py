...
def rolling_prediction(df, primary_target, targets, base_cols, top_n=30, train_window=1008, period_days=30):
    """滚动预测 - 4年窗口"""
    print("\n" + "=" * 60)
    print(f"30因子滚动预测 - 4年训练窗口（{train_window}天）")
    print("=" * 60)
...
def create_visualizations(results, output_dir):
    ...
    # 计算策略收益
    threshold = 0.05
    positions = np.sign(y_pred) * np.minimum(np.abs(y_pred), 1.0)
    positions = np.where(np.abs(positions) >= threshold, positions, 0)
    positions_shifted = np.roll(positions, 1)
    positions_shifted[0] = 0

    gross_returns = positions_shifted * y_true
    trade_costs = np.abs(np.diff(positions_shifted, prepend=0)) * 0.001
    ...
    ax1 = axes[0]
    ax1.plot(dates, cumulative * 100, 'b-', linewidth=1.5, label='策略累计收益')
    ax1.fill_between(dates, 0, cumulative * 100, alpha=0.3)
    ax1.axhline(y=0, color='k', linestyle='--', linewidth=0.5)
    ax1.set_ylabel('累计收益 (%)', fontsize=11)
    ax1.set_title('4年窗口 - 30因子原油风险预测模型 - 策略累计收益', fontsize=13, fontweight='bold')
    ...
    ax2.set_title('4年窗口 - 风险指数时间序列', fontsize=13, fontweight='bold')
    ...
    # 3. 多时间尺度预测方向
    ax3 = axes[2]
    pred_1d = np.array(results['predicted_1d'])
    pred_5d = np.array(results['predicted_5d'])
    pred_20d = np.array(results['predicted_20d'])

    dir_1d = np.sign(pred_1d)
    dir_5d = np.sign(pred_5d)
    dir_20d = np.sign(pred_20d)

    ax3.plot(dates, dir_1d, 'b-', linewidth=0.8, label='1天方向', alpha=0.7)
    ax3.plot(dates, dir_5d * 0.8, 'g-', linewidth=0.8, label='5天方向', alpha=0.7)
    ax3.plot(dates, dir_20d * 0.6, 'r-', linewidth=0.8, label='20天方向', alpha=0.7)
    ax3.axhline(y=0, color='k', linestyle='-', linewidth=0.5)
    ax3.fill_between(dates, -1, 1, alpha=0.1)
    ax3.set_ylabel('预测方向', fontsize=11)
    ax3.set_xlabel('日期', fontsize=11)
    ax3.set_title('4年窗口 - 多时间尺度预测方向对比', fontsize=13, fontweight='bold')
    ...
    axes[0].set_title('4年窗口 - 风险等级分布', fontsize=12, fontweight='bold')
    ...
    axes[1].set_title('4年窗口 - 年度收益表现', fontsize=12, fontweight='bold')
    ...
    axes[2].set_title('4年窗口 - 多时间尺度预测准确率', fontsize=12, fontweight='bold')
    axes[2].legend()
...
def evaluate_and_save(results, output_dir='final_solution_4year'):
    """评估并保存结果"""
    ...
    # 对比
    print("\n" + "=" * 60)
    print("对比分析（1年 vs 2年 vs 3年 vs 4年窗口）")
    print("=" * 60)
    print("| 指标      | 1年    | 2年    | 3年    | 4年    |")
    print("|-----------|--------|--------|--------|--------|")
    print(f"| 年化收益  |  3.38% |  3.95% |  8.06% | {annual_ret:5.2%} |")
    print(f"| 夏普比率  |  0.29  |  0.34  |  0.72  |  {sharpe:.2f}  |")
    print(f"| 最大回撤  | -42.4% | -43.9% | -31.1% | {max_dd:6.2%}|")
    print(f"| 1天准确率 |  50.5% |  52.0% |  53.2% |  {dir_acc_1d:5.2%}|")
    ...
    with open(f'{output_dir}/report.txt', 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("30因子原油风险预测模型 - 4年窗口版报告\n")
        f.write("=" * 80 + "\n\n")
        f.write("【模型配置】\n")
        f.write(f"  训练窗口: 1008天（4年）\n")
        f.write(f"  因子数量: 30个\n\n")
        f.write("【策略表现】\n")
        ...
        f.write("【窗口对比】\n")
        f.write("  窗口    年化    夏普    回撤\n")
        f.write("  1年    3.38%   0.29   -42%\n")
        f.write("  2年    3.95%   0.34   -44%\n")
        f.write("  3年    8.06%   0.72   -31%\n")
        f.write(f"  4年   {annual_ret:5.2%}   {sharpe:.2f}   {max_dd:3.0%}\n\n")
...
def main():
    ...
    output_dir = "final_solution_4year"

    df, primary_target, targets, base_cols = load_and_prepare_data(file_path)
    results = rolling_prediction(df, primary_target, targets, base_cols, train_window=1008)
